...
function queryBuilder(table, q = { select: '*', filters: [], limit: null, orderCol: null, orderDir: 'asc', method: null, body: null }) {
  const b = {
    ...
    neq(col, val) { q.filters.push({ col, val: `neq.${val}` }); return b; },
    is(col, val) { q.filters.push({ col, val: val === null ? 'is.null' : `is.${val}` }); return b; },
    not(col, op, val) { q.filters.push({ col, val: `not.${op}.${val === null ? 'null' : val}` }); return b; },
    in(col, vals) { q.filters.push({ col, val: `in.(${vals.join(',')})` }); return b; },
    lt(col, val) { q.filters.push({ col, val: `lt.${val}` }); return b; },
    gt(col, val) { q.filters.push({ col, val: `gt.${val}` }); return b; },
